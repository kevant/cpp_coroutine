/*
* In this example, the Awaiter will return true for await_ready (or std::suspend_never)
* This will cause the coroutine function to behave similar to a normal synchronous function,
* as it will not return control back to caller until it finished
* 
* Another notable point is that await_suspend() will never be invoked;
*/
...
struct Coro
{
    struct promise_type
    {
        ...
        Awaiter initial_suspend()
        {
            ...
            return Awaiter{ .suspend_never = true };
        }
...
Coro MyCoroutine()
{
    ...
    co_await Awaiter{ .suspend_never = true };
